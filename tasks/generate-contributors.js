...
const options = {
  url:
    "https://api.github.com/repos/jeanbauer/awesome-netherlands/stats/contributors",
  headers: {
    "User-Agent": "request"
  }
};
...
request(options, (error, response, body) => {
  ...
  filteredContributors.forEach(c =>
    console.log(`[${c.author.login}] ${computeContributorAdditions(c)}`)
  );
...
function handleHttpError(error, response, body) {
  throw `[${response &&
    response.statusCode}] Error while performing http request: ${error}\n${body}`;
}
